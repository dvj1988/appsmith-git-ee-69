export default {
	...
	getFilters() {
		...
		if (conditions.length > 0) {
			return "WHERE (" + conditions.join(') and (') + ") and status in ('New','Pending on L2','Pending on L1') and (slack_thread is not null or slack_thread != '') and (l2owner is NULL or l2owner = '')";
		} else {
			return "WHERE status in ('New','Pending on L2','Pending on L1') and (slack_thread is not null or slack_thread != '') and (l2owner is NULL or l2owner = '')";
		}
	}
}