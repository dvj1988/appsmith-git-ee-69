export default {
	...
	getSupportTier() {
		...
		let doms = filter_ticket_support_level.selectedOptionLabels;
		let conds = (filters.length > 0 ? filters.map(s => `org_id = ${s}`) : []);
		let conds2 = (doms.length > 0 ? doms.map(s => `requester_email ilike '%${s}'`) : []);
		return [...conds, ...conds2].join(' or ')
	},
