export default {
	...
	getEnterpriseMediumChart(bg, c) {
		const data = Utils.getEnterpriseIssues(Utils.getMediumIssues(get_resolution_time.data));
		return TimingChart.getTimingChart(data, bg, c, 24);
	},
	...
	getCommonMediumChart(bg, c) {
		const data = Utils.getMediumIssues(get_resolution_time.data);
		return TimingChart.getTimingChart(data, bg, c, 24);
	},
	getCommonNormalChart(bg, c) {
		const data = Utils.getNormalIssues(get_resolution_time.data);
		return TimingChart.getTimingChart(data, bg, c, 24);
	},
	getCommonLowChart(bg, c) {
		const data = Utils.getLowIssues(get_resolution_time.data);
		return TimingChart.getTimingChart(data, bg, c, 24);
	},
}